...
An existing SFAPI bearer token can be used directly instead. Store the raw token under the `bearer_token` key:
...
To use a custom raw Secret key, set `nersc.sf/credentialSecretKey` to that key. `sf_api.json` itself is reserved for client credentials; raw bearer tokens are always read from their own Secret key. Supplied bearer tokens are used as-is and must be rotated before they expire.

To import credentials already generated by the Globus CLI and NERSC tooling, see [Using local Globus CLI and SFAPI tokens](docs/local-credentials.md). The accompanying script reads `~/.globus/cli/storage.db` and `~/.ssh/nersc-token` without printing their contents and runs a stage-in smoke test.
  ...
  template:
    metadata:
      ...
      annotations:
        ...
        globus.api/inputSource: "globus://source-collection-id/path/to/data"
...
VK will:
1. Stage input data from `globus.api/inputSource` to the selected scratch staging path before Slurm job submission
2. Mount scratch paths in the container via `--volume`
3. Start output staging to `globus.api/outputDest` after the Slurm job succeeds when `nersc.sf/stageOut` is `true`
...
| `nersc.sf/credentialSecretKey` | No | Secret key containing SFAPI credential JSON or a raw bearer token; defaults to `sf_api.json`, with automatic fallback to `bearer_token`. |
...
| `nersc.sf/inputSource` | No | Provider-local input file under `SFAPI_TRANSFER_LOCAL_ROOT` in SFAPI mode. Also accepted as a legacy fallback in Globus mode. |
| `nersc.sf/outputDest` | Required for SFAPI stage-out | Provider-local destination file under `SFAPI_TRANSFER_LOCAL_ROOT`. Also accepted as a legacy fallback in Globus mode. |
...
| `globus.api/credentialSecretKey` | No | JSON key containing Globus credentials; defaults to `globus.json`. Separate `client_id`, `client_secret`, `bearer_token`, and `refresh_token` keys are supported. |
| `globus.api/inputSource` | No | `globus://<collection-uuid>/<collection-relative-path>` source for stage-in. |
| `globus.api/outputDest` | Required for Globus stage-out | `globus://<collection-uuid>/<collection-relative-path>` destination. |
...
- `run-local-globus-workflow.sh` — stage-in smoke test for the Globus staging integration using local Globus CLI and SFAPI tokens
