...
Bearer tokens can also be stored in `sf_api.json` as `{"bearer_token":"..."}`. To use any custom raw Secret key, set `nersc.sf/credentialSecretKey` to that key. Supplied bearer tokens are used as-is and must be rotated before they expire.
